...
public class ConnectManagerTest {
	...
	public void roundrobinは呼ぶ度に異なる() {
		...
		String out1 = request(group);
		String out2 = request(group);
		String out3 = request(group);
		String out4 = request(group);
		logger.info("- - - - - - - - - - - - - -");
		logger.info(out1);
		logger.info(out2);
		logger.info(out3);
		logger.info(out4);
		String out5 = request(group);
		logger.info("- - - - - - - - - - - - - -");
		logger.info(out5);
	...
	private String request(ServerGroup group){
		Connector con = connectManager.getConnector(group);
		logger.info(con.toString());
		
		Method method = this.getClass().getMethods()[0];
		String out = con.execute(method, new Object[]{group});
		logger.info(out);
		return out;
	}
}
